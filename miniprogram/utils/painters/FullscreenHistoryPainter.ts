// miniprogram/utils/painters/FullscreenHistoryPainter.ts

import { BasePainter } from "./BasePainter";
import { FeatureNote, Manager,  } from "../storage/index";
import { DrawUtils } from "../shared/DrawUtils";

/**全屏模式的最近测试趋势曲线绘制器 */
export class FullscreenHistoryPainter extends BasePainter {
  private historyData: [] = [];
  ...
  async drawFrame() {
    ...
    this.ctx.fillText("最近测试趋势", this.width / 2, 25);
    this.ctx.restore();
    ...
    // 6. 绘制每个数据点
    DrawUtils.drawHistoryNodesFor(this.ctx, data, {
  ...
  private async loadHistoryData() {
    ...
    try {
      ...
      if (this.currentFilter.bodyPart || this.currentFilter.action) {
        this.historyData = await Manager.querys({
        ...
        this.historyData = await Manager.getList(5);
        console.log("全屏MMT趋势数据加载成功:", this.historyData.length, "条");
      }
    } catch (error) {
      console.error("加载全屏MMT趋势数据失败:", error);
      this.historyData = [];
    } finally {
